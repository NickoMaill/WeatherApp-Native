...
class WeatherService {
    ...
    public async getCurrentWeatherByCoordinate(lon: number, lat: number, units: 'metric' | 'imperial' = 'metric', lang: 'en' | 'fr' = 'fr'): Promise<WeatherTypeDto> {
        const data = await apiManager.get<WeatherCurrentResponseApi>('weather',`weather?lat=${lat}&lon=${lon}&units=${units}&lang=${lang}&appid=${configManager.getConfig.WEATHER_API_KEY}`);
    ...
    public async getCurrentWeatherByCityId(cityId: number, units: 'metric' | 'imperial' = 'metric', lang: 'en' | 'fr' = 'fr'): Promise<WeatherTypeDto> {
        const data = await apiManager.get<WeatherCurrentResponseApi>('weather', `weather?id=${cityId}&units=${units}&lang=${lang}&appid=${configManager.getConfig.WEATHER_API_KEY}`);
        return this.formatCurrentData(data);
    }
